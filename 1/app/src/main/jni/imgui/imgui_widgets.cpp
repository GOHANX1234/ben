...
{  
    ...
    const float square_sz = GetFrameHeight() * 1.0f; 
    const ImVec2 pos = window->DC.CursorPos;  
    ...
    {  
        ...
        ImVec2 start = check_bb.Min + ImVec2(pad, pad + square_sz * 0.25f);  
        ImVec2 mid   = check_bb.Min + ImVec2(pad + square_sz * 0.25f, square_sz - pad);  
        ImVec2 end   = check_bb.Min + ImVec2(square_sz - pad, pad);  
  
        window->DrawList->AddLine(start, mid, check_col, 1.2f);  
        window->DrawList->AddLine(mid, end, check_col, 1.2f);  
    }  
