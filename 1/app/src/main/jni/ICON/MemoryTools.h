...
public:
    ...
    Addr readSafeAddr(Addr addr, int offset = 0);    //保护读取addr内存,如果缺页直接返回0 需要先执行初始化initPageMap()方法,//@Parth_Parth
    ...
    MemoryTools &dumpMem(Addr startAddr, Addr endAddr);     //持久化内存空间数据 可以链式//@Parth_Parth
